...
class MultiSeqQueryGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.user_gates_per_seq = nn.ModuleList([
            nn.Sequential(nn.Linear(3 * d_model, d_model), nn.Sigmoid())
            for _ in range(num_sequences)
        ])
        self.item_gates_per_seq = nn.ModuleList([
            nn.Sequential(nn.Linear(3 * d_model, d_model), nn.Sigmoid())
            for _ in range(num_sequences)
        ])
    ...
    def forward(
        ...
        for i in range(self.num_sequences):
            ...
            gated_user = user_summary * self.user_gates_per_seq[i](gate_input)
            gated_item = item_summary * self.item_gates_per_seq[i](gate_input)

            # GlobalInfo_i = Concat(Gate_i(F_u), Gate_i(F_i), MeanPool(Seq_i))
            global_info = torch.cat(
                [gated_user, gated_item, seq_pooled], dim=-1
            )  # (B, 3*D)
...
class PCVRHyFormer(nn.Module):
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        q_tokens_list = self.query_generator(
            user_tokens, item_tokens, seq_tokens_list, seq_masks_list)
