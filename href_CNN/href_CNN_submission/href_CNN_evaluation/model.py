...
class SeqCNNSummarizer(nn.Module):
    """Lightweight temporal CNN summary for one sequence domain."""
    ...
    def __init__(
        ...
        if kernel_size < 1 or kernel_size % 2 == 0:
            raise ValueError(f"seq_cnn_kernel must be a positive odd integer, got {kernel_size}")
        self.norm = nn.LayerNorm(d_model)
...
class MultiSeqQueryGenerator(nn.Module):
    ...
    def __init__(
        ...
        seq_summary_type: str = 'mean',
        seq_cnn_kernel: int = 5,
        dropout: float = 0.0,
    ) -> None:
        super().__init__()
        if seq_summary_type not in {'mean', 'cnn'}:
            raise ValueError(f"Unknown seq_summary_type: {seq_summary_type}")
        self.num_queries = num_queries
        self.num_sequences = num_sequences
        self.d_model = d_model
        self.num_user_ns = num_user_ns
        self.num_item_ns = num_item_ns
        self.seq_summary_type = seq_summary_type
        ...
        if seq_summary_type == 'cnn':
            self.seq_summarizers = nn.ModuleList([
                SeqCNNSummarizer(d_model, seq_cnn_kernel, dropout)
    ...
    def forward(
        ...
        for i in range(self.num_sequences):
            if self.seq_summary_type == 'cnn':
                seq_pooled = self.seq_summarizers[i](
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        self.query_generator = MultiSeqQueryGenerator(
            ...
            seq_summary_type=seq_summary_type,
            seq_cnn_kernel=seq_cnn_kernel,
