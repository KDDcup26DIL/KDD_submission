...
_FALLBACK_MODEL_CFG = {
    ...
    'use_click_user_init': False,
    'use_seq_user_init': True,
    'user_init_num_buckets': 131072,
    'user_init_dropout': 0.1,
    'user_init_gate': True,
}
...
def _batch_to_model_input(
    ...
    return ModelInput(
        ...
        user_ids=device_batch.get('user_id_tensor'),
    )
...
def main() -> None:
    ...
    # ---- Data loading: cap inference memory independently from training. ----
    # Training can use large batches with micro-batching, but inference runs a
    # full forward per DataLoader batch. With 512-token transformer sequences,
    # batch_size=256 can allocate >1GB for a single attention tensor.
    train_batch_size = int(train_config.get('batch_size', _FALLBACK_BATCH_SIZE))
    batch_size = min(train_batch_size, _MAX_EVAL_BATCH_SIZE)
    if os.environ.get('EVAL_BATCH_SIZE'):
        batch_size = int(os.environ['EVAL_BATCH_SIZE'])
    num_workers = int(os.environ.get('EVAL_NUM_WORKERS', _FALLBACK_NUM_WORKERS))
    logging.info(
        "Inference DataLoader config: batch_size=%d (train_batch_size=%d), "
        "num_workers=%d",
        batch_size, train_batch_size, num_workers,
    )
    ...
    use_amp = device == 'cuda' and os.environ.get('EVAL_USE_AMP', '1') != '0'
