...
class MultiSeqQueryGenerator(nn.Module):
    ...
    def __init__(
        ...
        user_init_dim: int = 0,
        user_init_dropout: float = 0.0,
        user_init_gate: bool = True,
    ) -> None:
        super().__init__()
        ...
        self.use_user_init = user_init_dim > 0
        self.user_init_gate_enabled = user_init_gate
        if self.use_user_init:
            self.user_init_proj = nn.Sequential(
                nn.Linear(user_init_dim, d_model),
                nn.LayerNorm(d_model),
            )
            self.user_init_dropout = nn.Dropout(user_init_dropout)
            if user_init_gate:
                self.user_init_gate = nn.Linear(2 * d_model, d_model)
            else:
                self.user_init_fusion = nn.Sequential(
    ...
    def forward(
        ...
        user_init: Optional[torch.Tensor] = None,
    ) -> list:
        ...
        if self.use_user_init and user_init is not None:
            init_summary = self.user_init_dropout(self.user_init_proj(user_init))
            if self.user_init_gate_enabled:
                gate = torch.sigmoid(
                    self.user_init_gate(torch.cat([user_summary, init_summary], dim=-1))
                )
                user_summary = gate * init_summary + (1.0 - gate) * user_summary
            else:
                user_summary = self.user_init_fusion(
                    torch.cat([user_summary, init_summary], dim=-1))
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        ns_summary_mode: str = 'mean',
        use_click_user_init: bool = False,
        use_seq_user_init: bool = True,
        user_init_num_buckets: int = 131072,
        user_init_dropout: float = 0.1,
        user_init_gate: bool = True,
        user_click_init_table: Optional[torch.Tensor] = None,
    ) -> None:
        super().__init__()
        ...
        self.use_click_user_init = use_click_user_init
        self.use_seq_user_init = use_seq_user_init
        self.user_init_num_buckets = int(user_init_num_buckets)
        ...
        # In-place dropout avoids allocating an additional large [B, L, D]
        # tensor for high-cardinality sequence id embeddings.
        self.seq_id_emb_dropout = nn.Dropout(dropout_rate * 2, inplace=True)
        ...
        self.query_generator = MultiSeqQueryGenerator(
            ...
            user_init_dim=d_model if (use_click_user_init or use_seq_user_init) else 0,
            user_init_dropout=user_init_dropout,
            user_init_gate=user_init_gate,
        )

        if use_click_user_init:
            if user_click_init_table is None:
                user_click_init_table = torch.zeros(
                    self.user_init_num_buckets, d_model, dtype=torch.float32)
            if user_click_init_table.shape != (self.user_init_num_buckets, d_model):
                raise ValueError(
                    f"user_click_init_table shape must be "
                    f"({self.user_init_num_buckets}, {d_model}), got "
                    f"{tuple(user_click_init_table.shape)}")
            self.register_buffer(
                'user_click_init_table',
                user_click_init_table.detach().float().clone(),
            )
        else:
            self.register_buffer(
                'user_click_init_table',
                torch.zeros(0, d_model, dtype=torch.float32),
                persistent=False,
            )
    ...
    def _lookup_user_click_init(
        self, user_ids: Optional[torch.Tensor]
    ) -> Optional[torch.Tensor]:
        if not self.use_click_user_init or user_ids is None:
            return None
        bucket_ids = torch.remainder(user_ids.long(), self.user_init_num_buckets)
        return self.user_click_init_table[bucket_ids]

    def _make_seq_user_init(
        self,
        seq_tokens_list: list,
        seq_masks_list: list,
    ) -> Optional[torch.Tensor]:
        if not self.use_seq_user_init:
            return None
        ...
        for seq_tokens, seq_mask in zip(seq_tokens_list, seq_masks_list):
            ...
            pooled_list.append((seq_tokens * valid).sum(dim=1))
            count_list.append(valid.sum(dim=1))

        total = torch.stack(pooled_list, dim=0).sum(dim=0)
        count = torch.stack(count_list, dim=0).sum(dim=0).clamp(min=1.0)
        return total / count

    def _resolve_user_init(
        self,
        user_ids: Optional[torch.Tensor],
        seq_tokens_list: list,
        seq_masks_list: list,
    ) -> Optional[torch.Tensor]:
        seq_init = self._make_seq_user_init(seq_tokens_list, seq_masks_list)
        click_init = self._lookup_user_click_init(user_ids)
        if seq_init is None:
            return click_init
        if click_init is None:
            return seq_init
        return 0.5 * (seq_init + click_init)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        user_init = self._resolve_user_init(inputs.user_ids, seq_tokens_list, seq_masks_list)
        q_tokens_list = self.query_generator(
            refined_user, refined_item, seq_tokens_list, seq_masks_list, user_init=user_init)
