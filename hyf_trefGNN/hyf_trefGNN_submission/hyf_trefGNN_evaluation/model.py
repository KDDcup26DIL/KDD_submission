...
class BatchLightGCNTower(nn.Module):
    ...
    def __init__(
        ...
        self.user_id_emb = nn.Embedding(user_buckets, d_model)
        self.item_id_emb = nn.Embedding(item_buckets, d_model)
    ...
    def forward(
        ...
        user_base = self.user_proj(user_repr) + self.user_id_emb(user_ids % self.user_buckets)
        item_base = self.item_proj(item_repr) + self.item_id_emb(item_ids % self.item_buckets)
...
class PCVRHyFormer(nn.Module):
    ...
    def forward(
        ...
        if self.use_graph_tower and graph_user_ids is not None and graph_item_ids is not None:
            ...
            logits = hyf_logits + self.graph_alpha * graph_scores.unsqueeze(-1)
    ...
    def predict(
        ...
        if self.use_graph_tower and graph_user_ids is not None and graph_item_ids is not None:
            ...
            logits = hyf_logits + self.graph_alpha * graph_out["scores"].unsqueeze(-1)
