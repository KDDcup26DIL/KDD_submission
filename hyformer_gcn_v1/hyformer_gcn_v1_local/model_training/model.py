...
class HashedCooccurrenceGCN(nn.Module):
    """LightGCN-style propagation over typed co-occurrence token buckets.

    Edges are built from input context only: sequence tokens that co-occur with
    the current item id in a row. Labels are not used to build this graph.
    """
    ...
    def __init__(
        ...
        num_layers: int = 1,
        edge_chunk_size: int = 131072,
    ) -> None:
        super().__init__()
        self.num_buckets = int(num_buckets)
        self.num_layers = int(num_layers)
        self.edge_chunk_size = max(1, int(edge_chunk_size))
        self.embedding = nn.Embedding(self.num_buckets, d_model, padding_idx=0)

        if edge_src is None:
            edge_src = torch.empty(0, dtype=torch.long)
        if edge_dst is None:
            edge_dst = torch.empty(0, dtype=torch.long)
        edge_src = edge_src.long().clamp(0, self.num_buckets - 1)
        edge_dst = edge_dst.long().clamp(0, self.num_buckets - 1)
        self.register_buffer('edge_src', edge_src, persistent=True)
        self.register_buffer('edge_dst', edge_dst, persistent=True)

        deg = torch.zeros(self.num_buckets, dtype=torch.float32)
        if edge_src.numel() > 0:
            ones = torch.ones(edge_src.numel(), dtype=torch.float32)
            deg.index_add_(0, edge_src.cpu(), ones)
            deg.index_add_(0, edge_dst.cpu(), ones)
        self.register_buffer('deg_inv_sqrt', deg.clamp(min=1.0).pow(-0.5), persistent=True)
    ...
    @staticmethod
    def hash_typed(values: torch.Tensor, type_id: int, num_buckets: int) -> torch.Tensor:
        values = values.long()
        valid = values > 0
        mixed = values.clamp_min(0) * 1_000_003 + int(type_id) * 9_176 + 2_654_435_761
        hashed = torch.remainder(mixed, max(1, num_buckets - 1)) + 1
        return torch.where(valid, hashed, torch.zeros_like(hashed)).clamp(0, num_buckets - 1)

    @staticmethod
    def item_type_id() -> int:
        return 1

    @staticmethod
    def seq_type_id(domain_idx: int, slot: int) -> int:
        return 1000 + int(domain_idx) * 100 + int(slot)

    def propagate(self) -> torch.Tensor:
        x = self.embedding.weight
        if self.edge_src.numel() == 0 or self.num_layers <= 0:
            return x
        ...
        for _ in range(self.num_layers):
            ...
            for start in range(0, src.numel(), self.edge_chunk_size):
                ...
                src_c = src[start:end]
                dst_c = dst[start:end]
                norm_c = norm[start:end].unsqueeze(-1)
                msg = x[src_c] * norm_c
                rev_msg = x[dst_c] * norm_c
                next_x.index_add_(0, dst_c, msg)
                next_x.index_add_(0, src_c, rev_msg)
            x = next_x
            xs.append(x)

        return torch.stack(xs, dim=0).mean(dim=0)

    def forward(self, raw_item_ids: torch.Tensor, seq_data: dict) -> Tuple[torch.Tensor, torch.Tensor]:
        all_nodes = self.propagate()
        item_idx = self.hash_typed(raw_item_ids, self.item_type_id(), self.num_buckets).to(all_nodes.device)
        item_emb = all_nodes[item_idx]

        seq_sum = torch.zeros_like(item_emb)
        seq_count = torch.zeros(item_emb.shape[0], 1, dtype=item_emb.dtype, device=item_emb.device)
        for domain_idx, domain in enumerate(sorted(seq_data.keys())):
            seq = seq_data[domain]
            # seq: [B, S, L]. Each feature slot is kept as a distinct typed token.
            for slot in range(seq.shape[1]):
                vals = seq[:, slot, :]
                idx = self.hash_typed(vals, self.seq_type_id(domain_idx, slot), self.num_buckets).to(all_nodes.device)
                mask = idx > 0
                emb = all_nodes[idx]
                seq_sum = seq_sum + (emb * mask.unsqueeze(-1)).sum(dim=1)
                seq_count = seq_count + mask.sum(dim=1, keepdim=True).to(seq_count.dtype)
        seq_emb = seq_sum / seq_count.clamp(min=1.0)
        return item_emb, seq_emb
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        if self.use_lightgcn:
            if gcn_edge_src is None:
                gcn_edge_src = gcn_edge_users
            if gcn_edge_dst is None:
                gcn_edge_dst = gcn_edge_items
            self.lightgcn = HashedCooccurrenceGCN(
                num_buckets=gcn_num_buckets or gcn_user_buckets,
                d_model=d_model,
                edge_src=gcn_edge_src,
                edge_dst=gcn_edge_dst,
                num_layers=gcn_num_layers,
                edge_chunk_size=gcn_edge_chunk_size,
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        if self.use_lightgcn:
            item_gcn_emb, user_gcn_emb = self.lightgcn(inputs.item_ids, inputs.seq_data)
