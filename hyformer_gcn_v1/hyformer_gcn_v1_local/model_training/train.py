...
def _hash_typed_np(ids: np.ndarray, type_id: int, num_buckets: int) -> np.ndarray:
    ids = ids.astype(np.int64, copy=False)
    mixed = np.maximum(ids, 0) * 1_000_003 + int(type_id) * 9_176 + 2_654_435_761
    out = (mixed % max(1, num_buckets - 1)) + 1
    out[ids <= 0] = 0
    return out.astype(np.int64, copy=False)


def _seq_type_id(domain_idx: int, slot: int) -> int:
    return 1000 + int(domain_idx) * 100 + int(slot)


def build_hashed_cooccurrence_graph(
    data_dir: str,
    schema_path: str,
    output_path: str,
    num_buckets: int,
    max_seq_tokens_per_row: int,
    max_edges: int,
    valid_ratio: float,
    train_ratio: float,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Build unique label-free sequence-token/current-item co-occurrence edges."""
    with open(schema_path, 'r', encoding='utf-8') as f:
        schema = json.load(f)

    seq_cols = []
    for domain_idx, domain in enumerate(sorted(schema['seq'].keys())):
        cfg = schema['seq'][domain]
        prefix = cfg['prefix']
        ts_fid = cfg['ts_fid']
        sideinfo = [fid for fid, _ in cfg['features'] if fid != ts_fid]
        for slot, fid in enumerate(sideinfo):
            seq_cols.append((f'{prefix}_{fid}', _seq_type_id(domain_idx, slot)))

    rg_info = []
    for path in sorted(glob.glob(os.path.join(data_dir, '*.parquet'))):
        pf = pq.ParquetFile(path)
        for rg_idx in range(pf.metadata.num_row_groups):
            rg_info.append((path, rg_idx))
    ...
    edge_chunks = []
    raw_edge_count = 0
    raw_edge_limit = max_edges * 4 if max_edges > 0 else 0
    stop_build = False
    for path in sorted({p for p, _ in train_rg_info}):
        if stop_build:
            break
        pf = pq.ParquetFile(path)
        row_groups = [rg for p, rg in train_rg_info if p == path]
        columns = ['item_id'] + [name for name, _ in seq_cols]
        for batch in pf.iter_batches(columns=columns, row_groups=row_groups):
            if stop_build:
                break
            B = batch.num_rows
            item_ids = batch.column('item_id').fill_null(0).to_numpy(
                zero_copy_only=False).astype(np.int64)
            item_nodes = _hash_typed_np(item_ids, 1, num_buckets)

            per_row_src = [[] for _ in range(B)]
            for col_name, type_id in seq_cols:
                col = batch.column(col_name)
                offsets = col.offsets.to_numpy()
                values = col.values.to_numpy(zero_copy_only=False).astype(np.int64)
                for i in range(B):
                    if item_nodes[i] <= 0 or len(per_row_src[i]) >= max_seq_tokens_per_row:
                        continue
                    start = int(offsets[i])
                    end = int(offsets[i + 1])
                    if end <= start:
                    ...
                    if vals.size == 0:
                        continue
                    remain = max_seq_tokens_per_row - len(per_row_src[i])
                    nodes = _hash_typed_np(vals[:remain], type_id, num_buckets)
                    per_row_src[i].extend(nodes[nodes > 0].tolist())

            src_all = []
            dst_all = []
            for i, src_nodes in enumerate(per_row_src):
                if item_nodes[i] <= 0 or not src_nodes:
                    continue
                src_all.extend(src_nodes)
                dst_all.extend([int(item_nodes[i])] * len(src_nodes))
            if src_all:
                edge_chunks.append(np.stack([
                    np.asarray(src_all, dtype=np.int64),
                    np.asarray(dst_all, dtype=np.int64),
                ], axis=1))
                raw_edge_count += len(src_all)
                if raw_edge_limit > 0 and raw_edge_count >= raw_edge_limit:
                    stop_build = True
    ...
    if edge_chunks:
        ...
        edge_src = torch.from_numpy(edges[:, 0].astype(np.int64))
        edge_dst = torch.from_numpy(edges[:, 1].astype(np.int64))
    else:
        edge_src = torch.empty(0, dtype=torch.long)
        edge_dst = torch.empty(0, dtype=torch.long)

    torch.save({'edge_src': edge_src, 'edge_dst': edge_dst}, output_path)
    logging.info(
        f"Built co-occurrence GCN graph: {edge_src.numel()} unique edges, "
        f"node_buckets={num_buckets}, max_seq_tokens_per_row={max_seq_tokens_per_row}, "
        f"max_edges={max_edges}, raw_edges_seen={raw_edge_count}, "
        f"train_row_groups={n_train_rgs}/{total_rgs}, "
        f"path={output_path}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--use_lightgcn', action='store_true', default=True,
                        help='Enable label-free hashed co-occurrence GCN embeddings')
    parser.add_argument('--no_lightgcn', dest='use_lightgcn', action='store_false',
                        help='Disable hashed co-occurrence GCN embeddings')
    parser.add_argument('--gcn_num_buckets', type=int, default=65536,
                        help='Number of hashed typed-token buckets for co-occurrence GCN')
    parser.add_argument('--gcn_user_buckets', type=int, default=65536,
                        help='Deprecated alias retained for old configs')
    parser.add_argument('--gcn_item_buckets', type=int, default=65536,
                        help='Deprecated alias retained for old configs')
    parser.add_argument('--gcn_num_layers', type=int, default=1,
                        help='Number of co-occurrence GCN propagation layers')
    parser.add_argument('--gcn_edge_chunk_size', type=int, default=32768,
                        help='Number of graph edges processed per propagation chunk')
    parser.add_argument('--gcn_seq_max_tokens_per_row', type=int, default=16,
                        help='Max sequence typed tokens connected to the current item per row')
    parser.add_argument('--gcn_max_edges', type=int, default=200000,
                        help='Max unique graph edges kept after deterministic sampling (<=0 keeps all)')
    parser.add_argument('--gcn_graph_path', type=str, default=None,
                        help='Optional path for the serialized hashed co-occurrence graph')
...
def main() -> None:
    ...
    if args.use_lightgcn:
        ...
        gcn_edge_src, gcn_edge_dst = build_hashed_cooccurrence_graph(
            data_dir=args.data_dir,
            schema_path=schema_path,
            output_path=gcn_graph_path,
            num_buckets=args.gcn_num_buckets,
            max_seq_tokens_per_row=args.gcn_seq_max_tokens_per_row,
            max_edges=args.gcn_max_edges,
            valid_ratio=args.valid_ratio,
            train_ratio=args.train_ratio,
        )
    ...
    model_args = {
        ...
        "gcn_num_buckets": args.gcn_num_buckets,
        "gcn_user_buckets": args.gcn_user_buckets,
        "gcn_item_buckets": args.gcn_item_buckets,
        "gcn_num_layers": args.gcn_num_layers,
        "gcn_edge_chunk_size": args.gcn_edge_chunk_size,
        "gcn_edge_src": gcn_edge_src,
        "gcn_edge_dst": gcn_edge_dst,
    }
