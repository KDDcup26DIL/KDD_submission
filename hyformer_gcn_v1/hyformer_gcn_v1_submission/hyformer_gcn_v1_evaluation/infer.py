...
_FALLBACK_MODEL_CFG = {
    ...
    'item_ns_tokens': 0,
    'use_lightgcn': False,
    'gcn_num_buckets': 65536,
    'gcn_user_buckets': 65536,
    'gcn_item_buckets': 65536,
    'gcn_num_layers': 1,
    'gcn_edge_chunk_size': 131072,
}
...
def build_model(
    ...
    gcn_graph_path: Optional[str] = None,
    device: str = 'cpu',
) -> PCVRHyFormer:
    ...
    if model_cfg.get('use_lightgcn', False):
        if not gcn_graph_path or not os.path.exists(gcn_graph_path):
            raise FileNotFoundError(
                f"use_lightgcn=True but gcn_graph.pt was not found: {gcn_graph_path}")
        graph = torch.load(gcn_graph_path, map_location='cpu')
        model_cfg = dict(model_cfg)
        model_cfg['gcn_edge_src'] = graph.get('edge_src', graph.get('edge_users'))
        model_cfg['gcn_edge_dst'] = graph.get('edge_dst', graph.get('edge_items'))

    log_cfg = {
        k: (f"Tensor(shape={tuple(v.shape)}, dtype={v.dtype})"
            if isinstance(v, torch.Tensor) else v)
        for k, v in model_cfg.items()
    }
    logging.info(f"Building PCVRHyFormer with cfg: {log_cfg}")
    model = PCVRHyFormer(
...
def _batch_to_model_input(
    ...
    return ModelInput(
        ...
        user_ids=device_batch['user_id_tensor'],
        item_ids=device_batch['item_id_tensor'],
        seq_data=seq_data,
        seq_lens=seq_lens,
...
def main() -> None:
    ...
    gcn_graph_path = train_config.get('gcn_graph_path', None)
    if gcn_graph_path:
        local_candidate = os.path.join(model_dir, os.path.basename(gcn_graph_path))
        if os.path.exists(local_candidate):
            gcn_graph_path = local_candidate

    model = build_model(
        test_dataset,
        model_cfg=model_cfg,
        ns_groups_json=ns_groups_json,
        gcn_graph_path=gcn_graph_path,
        device=device,
    )
