...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        self.gcn_graph_path: Optional[str] = gcn_graph_path
    ...
    def _write_sidecar_files(self, ckpt_dir: str) -> None:
        ...
        gcn_graph_copied = False
        if self.gcn_graph_path and os.path.exists(self.gcn_graph_path):
            shutil.copy2(self.gcn_graph_path, ckpt_dir)
            gcn_graph_copied = True

        if self.train_config:
            import json
            cfg_to_dump = self.train_config
            if ns_groups_copied or gcn_graph_copied:
                ...
                if ns_groups_copied:
                    cfg_to_dump['ns_groups_json'] = os.path.basename(
                        self.ns_groups_path)
                if gcn_graph_copied:
                    cfg_to_dump['gcn_graph_path'] = os.path.basename(
                        self.gcn_graph_path)
