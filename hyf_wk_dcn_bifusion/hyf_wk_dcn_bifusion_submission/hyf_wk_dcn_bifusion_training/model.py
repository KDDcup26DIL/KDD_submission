...
class CrossNetV2(nn.Module):
    """Small DCNv2 CrossNet over a compressed shared-token representation."""
...
class DCNv2LiteTower(nn.Module):
    ...
    This tower reuses the shared field tokens from FeatureTokenizer rather than
    creating another set of sparse embeddings. To keep total parameters close
    to the HyFormer+WuKong baseline, it first compresses the flattened token
    matrix to a small `dcn_dim`, then applies a shallow CrossNetV2 and a small
    parallel MLP before projecting back to `d_model`.
    """
    ...
    def __init__(
        ...
        self.crossnet = CrossNetV2(dcn_dim, max(1, num_layers))
...
class TriBilinearFusion(nn.Module):
    """Bilinear fusion for HyFormer, WuKong, and DCNv2-lite towers."""
    ...
    def __init__(
        ...
        self.dcn_norm = nn.LayerNorm(d_model)

        self.hy_linear = nn.Linear(d_model, action_num)
        self.wk_linear = nn.Linear(d_model, action_num)
        self.dcn_linear = nn.Linear(d_model, action_num)

        self.hy_proj = nn.Linear(d_model, num_heads * rank)
        self.wk_proj = nn.Linear(d_model, num_heads * rank)
        self.dcn_proj = nn.Linear(d_model, num_heads * rank)
        self.interaction_norm = nn.LayerNorm(num_heads * 3)
        self.interaction_dropout = nn.Dropout(dropout_rate)
        self.interaction_out = nn.Linear(num_heads * 3, action_num)
        self.interaction_scale = nn.Parameter(torch.tensor(0.05))

        self.rep_proj = nn.Sequential(
            nn.Linear(d_model * 3 + num_heads * 3, d_model),
            nn.LayerNorm(d_model),
            nn.GELU(),
        )

    def _pair_interaction(self, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        a = a.view(-1, self.num_heads, self.rank)
        b = b.view(-1, self.num_heads, self.rank)
        return (a * b).sum(dim=-1) / math.sqrt(float(self.rank))
    ...
    def forward(
        ...
        dcn_n = self.dcn_norm(dcn)

        hy_f = self.hy_proj(hy_n)
        wk_f = self.wk_proj(wk_n)
        dcn_f = self.dcn_proj(dcn_n)
        interactions = torch.cat([
            self._pair_interaction(hy_f, wk_f),
            self._pair_interaction(hy_f, dcn_f),
            self._pair_interaction(wk_f, dcn_f),
        ], dim=-1)
        interactions = self.interaction_norm(interactions)
        interactions = self.interaction_dropout(interactions)

        logits = (
            self.hy_linear(hy_n)
            + self.wk_linear(wk_n)
            + self.dcn_linear(dcn_n)
            + self.interaction_scale * self.interaction_out(interactions)
        )
        fused_rep = self.rep_proj(torch.cat([hy_n, wk_n, dcn_n, interactions], dim=-1))
...
class PCVRHyFormerWuKong(nn.Module):
    """Two-tower model: HyFormer-style transformer tower + WuKong tower with bilinear fusion."""
    ...
    def __init__(
        ...
        logging.info(
            ...
            "dcn_dim=%d, dcn_layers=%d, bifusion_rank=%d, bifusion_heads=%d",
            num_fields, d_model, emb_dim, num_hyformer_layers, num_wukong_layers,
            lcb_features, fmb_features, dcn_lite_dim, dcn_lite_layers,
            fusion_rank, fusion_heads,
        )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        logits, _ = self.fusion(hy, wk, dcn)
        return logits
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        wk = self.wukong_tower(tokens)
        dcn = self.dcn_tower(tokens)
        logits, fused = self.fusion(hy, wk, dcn)
        return logits, fused
