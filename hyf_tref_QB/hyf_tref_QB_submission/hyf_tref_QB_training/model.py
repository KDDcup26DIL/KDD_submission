...
class ModelInput(NamedTuple):
    ...
    user_int_qbins: Optional[torch.Tensor] = None
    item_int_qbins: Optional[torch.Tensor] = None
...
class NSTokenSummarizer(nn.Module):
    """Summarizes a variable number of NS tokens into one D-dimensional vector."""

    def __init__(self, d_model: int, mode: str = 'mean') -> None:
        super().__init__()
        if mode not in {'mean', 'attention'}:
            raise ValueError(f"Unknown ns_summary_mode: {mode}")
        self.mode = mode
        self.norm = nn.LayerNorm(d_model)
        if mode == 'attention':
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        if self.mode == 'mean':
            ...
            summary = (tokens * weights).sum(dim=1)
        return self.norm(summary)


class NSSetRefiner(nn.Module):
    """Refines user/item NS token sets before they are summarized for queries."""
    ...
    def __init__(
        ...
        self.summarizer = NSTokenSummarizer(d_model, summary_mode)
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        return self.summarizer(tokens).unsqueeze(1)


class BatchLightGCNTower(nn.Module):
    """Batch-local LightGCN tower with in-batch BPR negatives.

    This auxiliary variant intentionally does not use ID embeddings. The graph
    tower trains the refined feature representations F_u/F_i without giving the
    model a direct user/item ID memorization shortcut.
    """

    def __init__(
        self,
        d_model: int,
        user_buckets: int = 262144,
        item_buckets: int = 262144,
        num_layers: int = 1,
        ...
        self.user_proj = nn.Sequential(nn.Linear(d_model, d_model), nn.LayerNorm(d_model))
        self.item_proj = nn.Sequential(nn.Linear(d_model, d_model), nn.LayerNorm(d_model))
    ...
    def _compact_mean(
        ...
        out = reps.new_zeros(num_nodes, reps.shape[-1])
        out.index_add_(0, inverse, reps)
        counts = torch.bincount(inverse, minlength=num_nodes).clamp(min=1)
        return out / counts.unsqueeze(-1).to(out.dtype)
    ...
    def forward(
        ...
        user_ids = user_ids.long().clamp(min=0)
        item_ids = item_ids.long().clamp(min=0)
        user_base = self.user_proj(user_repr)
        item_base = self.item_proj(item_repr)

        uniq_users, user_inv = torch.unique(user_ids, sorted=False, return_inverse=True)
        uniq_items, item_inv = torch.unique(item_ids, sorted=False, return_inverse=True)
        del uniq_users, uniq_items
        user_nodes = self._compact_mean(user_base, user_inv, int(user_inv.max().item()) + 1)
        item_nodes = self._compact_mean(item_base, item_inv, int(item_inv.max().item()) + 1)

        pos_mask = labels is not None and bool((labels > 0.5).any().item())
        if pos_mask:
            labels_float = labels.float()
            pos_rows = torch.nonzero(labels_float > 0.5, as_tuple=False).squeeze(-1)
            pos_user = user_inv[pos_rows]
            pos_item = item_inv[pos_rows]
        else:
            pos_rows = user_ids.new_empty(0)
            pos_user = user_ids.new_empty(0)
            pos_item = item_ids.new_empty(0)

        user_layers = [user_nodes]
        item_layers = [item_nodes]
        if pos_user.numel() > 0:
            deg_user = user_nodes.new_zeros(user_nodes.shape[0])
            deg_item = item_nodes.new_zeros(item_nodes.shape[0])
            ones = user_nodes.new_ones(pos_user.shape[0])
            deg_user.index_add_(0, pos_user, ones)
            deg_item.index_add_(0, pos_item, ones)
            norm = torch.rsqrt((deg_user[pos_user] * deg_item[pos_item]).clamp(min=1.0))

            curr_user = user_nodes
            curr_item = item_nodes
            for _ in range(self.num_layers):
                next_user = torch.zeros_like(curr_user)
                next_item = torch.zeros_like(curr_item)
                next_user.index_add_(0, pos_user, curr_item[pos_item] * norm.unsqueeze(-1))
                next_item.index_add_(0, pos_item, curr_user[pos_user] * norm.unsqueeze(-1))
                curr_user, curr_item = next_user, next_item
                user_layers.append(curr_user)
                item_layers.append(curr_item)

        final_user_nodes = torch.stack(user_layers, dim=0).mean(dim=0)
        final_item_nodes = torch.stack(item_layers, dim=0).mean(dim=0)
        batch_user = final_user_nodes[user_inv]
        batch_item = final_item_nodes[item_inv]
        scores = (batch_user * batch_item).sum(dim=-1) / math.sqrt(self.d_model)

        bpr_loss = scores.new_zeros(())
        if labels is not None:
            pos_rows = torch.nonzero(labels.float() > 0.5, as_tuple=False).squeeze(-1)
            neg_rows = torch.nonzero(labels.float() <= 0.5, as_tuple=False).squeeze(-1)
            if pos_rows.numel() > 0 and neg_rows.numel() > 0:
                sample_idx = torch.randint(
                    low=0,
                    high=neg_rows.numel(),
                    size=(pos_rows.numel(), self.num_negatives),
                    device=scores.device,
                )
                neg_sample_rows = neg_rows[sample_idx]
                pos_scores = (batch_user[pos_rows] * batch_item[pos_rows]).sum(dim=-1)
                neg_scores = (
                    batch_user[pos_rows].unsqueeze(1) * batch_item[neg_sample_rows]
                ).sum(dim=-1)
...
class MultiSeqQueryGenerator(nn.Module):
    ...
    For each sequence i:
        GlobalInfo_i = Concat(F_u, F_i, MeanPool(Seq_i))
    ...
    def __init__(
        ...
        if num_user_ns <= 0 or num_item_ns <= 0:
            raise ValueError(
                f"num_user_ns and num_item_ns must be positive, got "
                f"{num_user_ns}, {num_item_ns}")

        self.user_summarizer = NSTokenSummarizer(d_model, ns_summary_mode)
        self.item_summarizer = NSTokenSummarizer(d_model, ns_summary_mode)

        global_info_dim = 3 * d_model
    ...
    def forward(
        ...
        Args:
            user_tokens: (B, M_u, D), user-side NS tokens.
            item_tokens: (B, M_i, D), item-side NS tokens.
        ...
        """
        user_summary = self.user_summarizer(user_tokens)  # (B, D)
        item_summary = self.item_summarizer(item_tokens)  # (B, D)
        ...
        for i in range(self.num_sequences):
            ...
            global_info = torch.cat(
                [user_summary, item_summary, seq_pooled], dim=-1
            )  # (B, 3*D)
...
class GroupNSTokenizer(nn.Module):
    ...
    def __init__(self, feature_specs: List[Tuple[int, int, int]],
                 ...
                 qbin_num_bins: int = 0) -> None:
        super().__init__()
        self.feature_specs = feature_specs
        self.groups = groups
        self.emb_dim = emb_dim
        self.emb_skip_threshold = emb_skip_threshold
        self.qbin_num_bins = qbin_num_bins
        ...
        qbin_embs = []
        for _, _, length in feature_specs:
            if qbin_num_bins > 0 and length == 1:
                qbin_embs.append(nn.Embedding(qbin_num_bins + 1, emb_dim, padding_idx=0))
            else:
                qbin_embs.append(None)
        self.qbin_embs = nn.ModuleList([e for e in qbin_embs if e is not None])
        self._qbin_emb_index = []
        real_idx = 0
        for e in qbin_embs:
            if e is not None:
                self._qbin_emb_index.append(real_idx)
                real_idx += 1
            else:
                self._qbin_emb_index.append(-1)
    ...
    def forward(
        ...
        int_qbins: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        for group, proj in zip(self.groups, self.group_projs):
            ...
            for fid_idx in group:
                ...
                qbin_real_idx = self._qbin_emb_index[fid_idx]
                if int_qbins is not None and qbin_real_idx != -1:
                    qbin_layer = self.qbin_embs[qbin_real_idx]
                    fid_emb = fid_emb + qbin_layer(int_qbins[:, offset].long())
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        ns_summary_mode: str = 'mean',
        use_graph_tower: bool = True,
        graph_user_buckets: int = 262144,
        graph_item_buckets: int = 262144,
        graph_layers: int = 1,
        graph_negatives: int = 10,
        graph_alpha: float = 0.0,
        bpr_weight: float = 0.01,
        qbin_num_bins: int = 0,
    ) -> None:
        super().__init__()
        ...
        self.use_graph_tower = use_graph_tower
        ...
        self.user_ns_refiner = NSSetRefiner(
            d_model=d_model,
            num_heads=num_heads,
            hidden_mult=hidden_mult,
            summary_mode=ns_summary_mode,
        )
        self.item_ns_refiner = NSSetRefiner(
            d_model=d_model,
            num_heads=num_heads,
            hidden_mult=hidden_mult,
            summary_mode=ns_summary_mode,
        )
        if self.use_graph_tower:
            self.graph_tower = BatchLightGCNTower(
                d_model=d_model,
                user_buckets=graph_user_buckets,
                item_buckets=graph_item_buckets,
                num_layers=graph_layers,
                num_negatives=graph_negatives,
            )
        ...
        self.query_generator = MultiSeqQueryGenerator(
            ...
            hidden_mult=hidden_mult,
            ns_summary_mode=ns_summary_mode,
        )
    ...
    def forward(
        self,
        inputs: ModelInput,
        graph_user_ids: Optional[torch.Tensor] = None,
        graph_item_ids: Optional[torch.Tensor] = None,
        ...
        user_ns = self.user_ns_tokenizer(
            inputs.user_int_feats, inputs.user_int_qbins)   # (B, num_user_groups, D)
        item_ns = self.item_ns_tokenizer(
            inputs.item_int_feats, inputs.item_int_qbins)   # (B, num_item_groups, D)
        ...
        ns_tokens = torch.cat([user_tokens, item_tokens], dim=1)  # (B, num_ns, D)
        ...
        refined_user = self.user_ns_refiner(user_tokens)
        refined_item = self.item_ns_refiner(item_tokens)
        q_tokens_list = self.query_generator(
            refined_user, refined_item, seq_tokens_list, seq_masks_list)
        ...
        # 5. Classifier
        hyf_logits = self.clsfier(output)  # (B, action_num)
        logits = hyf_logits
        graph_scores = None
        bpr_loss = hyf_logits.new_zeros(())
        if self.use_graph_tower and graph_user_ids is not None and graph_item_ids is not None:
            graph_out = self.graph_tower(
                refined_user.squeeze(1),
                refined_item.squeeze(1),
                graph_user_ids,
                graph_item_ids,
                graph_labels,
            )
            graph_scores = graph_out["scores"]
            bpr_loss = graph_out["bpr_loss"]

        if return_aux:
            return {
                "logits": logits,
                "hyf_logits": hyf_logits,
    ...
    def predict(
        ...
        user_ns = self.user_ns_tokenizer(inputs.user_int_feats, inputs.user_int_qbins)
        item_ns = self.item_ns_tokenizer(inputs.item_int_feats, inputs.item_int_qbins)
        ...
        hyf_logits = self.clsfier(output)
        logits = hyf_logits
