...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        qbin_stats: Optional[Dict[str, Any]] = None,
    ) -> None:
        """
        Args:
            ...
            qbin_stats: Optional quantile-boundary stats for scalar int
                features. When present, scalar int values produce companion
                qbin ids in ``user_int_qbins`` / ``item_int_qbins``.
        """
        super().__init__()
        ...
        self._buf_user_qbin = np.zeros((B, self.user_int_schema.total_dim), dtype=np.int64)
        self._buf_item_qbin = np.zeros((B, self.item_int_schema.total_dim), dtype=np.int64)
        ...
        for fid, vs, dim in self._user_int_cols:
            ...
            boundaries = self._get_qbin_boundaries('user_int', fid, dim)
            self._user_int_plan.append((ci, dim, offset, vs, boundaries))
            offset += dim
        ...
        for fid, vs, dim in self._item_int_cols:
            ...
            boundaries = self._get_qbin_boundaries('item_int', fid, dim)
            self._item_int_plan.append((ci, dim, offset, vs, boundaries))
            offset += dim
    ...
    def _get_qbin_boundaries(
        self,
        group: str,
        fid: int,
        dim: int,
    ) -> Optional["npt.NDArray[np.int64]"]:
        """Return scalar-feature qbin boundaries, or ``None`` when disabled."""
        if dim != 1:
            return None
        values = self.qbin_stats.get(group, {}).get(str(fid))
        if not values:
            return None
    ...
    def _make_qbins(
        self,
        arr: "npt.NDArray[np.int64]",
        boundaries: Optional["npt.NDArray[np.int64]"],
    ) -> "npt.NDArray[np.int64]":
        """Map positive scalar int values to 1..num_bins; 0 stays padding."""
        if boundaries is None:
            return np.zeros(arr.shape, dtype=np.int64)
        qbins = np.zeros(arr.shape, dtype=np.int64)
        valid = arr > 0
        if valid.any():
            qbins[valid] = np.searchsorted(
                boundaries, arr[valid], side='right') + 1
        return qbins
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        user_id_arr = batch.column(self._col_idx['user_id']).fill_null(0).to_numpy(
            zero_copy_only=False).astype(np.int64)
        item_id_arr = batch.column(self._col_idx['item_id']).fill_null(0).to_numpy(
            zero_copy_only=False).astype(np.int64)
        user_ids = user_id_arr.tolist()
        ...
        user_qbin = self._buf_user_qbin[:B]
        user_int[:] = 0
        user_qbin[:] = 0
        for ci, dim, offset, vs, boundaries in self._user_int_plan:
            col = batch.column(ci)
            if dim == 1:
                ...
                if vs > 0:
                    ...
                    arr[:] = 0
                user_int[:, offset] = arr
                user_qbin[:, offset] = self._make_qbins(arr, boundaries)
            else:
        ...
        item_qbin = self._buf_item_qbin[:B]
        item_int[:] = 0
        item_qbin[:] = 0
        for ci, dim, offset, vs, boundaries in self._item_int_plan:
            col = batch.column(ci)
            if dim == 1:
                arr = col.fill_null(0).to_numpy(zero_copy_only=False).astype(np.int64)
                arr[arr <= 0] = 0
                if vs > 0:
                    ...
                    arr[:] = 0
                item_int[:, offset] = arr
                item_qbin[:, offset] = self._make_qbins(arr, boundaries)
            else:
                padded, _ = self._pad_varlen_int_column(col, dim, B)
                if vs > 0:
        ...
        result = {
            ...
            'user_int_qbins': torch.from_numpy(user_qbin.copy()),
            'user_dense_feats': torch.from_numpy(user_dense.copy()),
            'item_int_feats': torch.from_numpy(item_int.copy()),
            'item_int_qbins': torch.from_numpy(item_qbin.copy()),
            'item_dense_feats': torch.zeros(B, 0, dtype=torch.float32),
            'label': torch.from_numpy(labels),
            'timestamp': torch.from_numpy(timestamps),
            'user_id': user_ids,
            'graph_user_ids': torch.from_numpy(user_id_arr.copy()),
            'graph_item_ids': torch.from_numpy(item_id_arr.copy()),
...
def build_quantile_bin_stats(
    ...
    """Estimate scalar int-feature quantile boundaries from training rows.

    Only scalar ``user_int`` / ``item_int`` features are considered. Multi-value
    int features receive qbin id 0 in ``PCVRParquetDataset``.
    """
    if num_bins <= 0:
        return {"version": 1, "num_bins": 0, "user_int": {}, "item_int": {}}

    with open(schema_path, 'r', encoding='utf-8') as f:
        raw_schema = json.load(f)

    scalar_user = [(int(fid), int(vs)) for fid, vs, dim in raw_schema['user_int']
                   if int(dim) == 1 and int(vs) > 0]
    scalar_item = [(int(fid), int(vs)) for fid, vs, dim in raw_schema['item_int']
                   if int(dim) == 1 and int(vs) > 0]
    ...
    for file_path in pq_files:
        ...
        for rg_idx in range(pf.metadata.num_row_groups):
            rg_info.append((file_path, rg_idx))
    ...
    samples: Dict[str, Dict[int, "npt.NDArray[np.int64]"]] = {
        'user_int': {fid: np.empty(0, dtype=np.int64) for fid, _ in scalar_user},
        'item_int': {fid: np.empty(0, dtype=np.int64) for fid, _ in scalar_item},
    }

    feature_specs = {
        'user_int': scalar_user,
        'item_int': scalar_item,
    }
    col_prefix = {
        'user_int': 'user_int_feats',
        'item_int': 'item_int_feats',
    }

    for file_path, rg_idx in train_rgs:
        pf = pq.ParquetFile(file_path)
        schema_names = pf.schema_arrow.names
        col_names = []
        for group, specs in feature_specs.items():
            for fid, _ in specs:
                name = f"{col_prefix[group]}_{fid}"
            ...
            for group, specs in feature_specs.items():
                for fid, vs in specs:
                    ...
                    if name not in batch_names:
                        continue
                    arr = batch.column(batch_names.index(name)).fill_null(0).to_numpy(
                        zero_copy_only=False).astype(np.int64)
                    arr = arr[(arr > 0) & (arr < vs)]
                    if arr.size == 0:
                        continue
                    cur = samples[group][fid]
                    merged = np.concatenate([cur, arr])
                    if merged.size > sample_size:
                        idx = rng.choice(merged.size, size=sample_size, replace=False)
                        merged = merged[idx]
                    samples[group][fid] = merged

    quantiles = np.linspace(0, 1, num_bins + 1, dtype=np.float64)[1:-1]
    stats: Dict[str, Any] = {
        "version": 1,
        "method": "reservoir_quantile",
    ...
    for group in ('user_int', 'item_int'):
        for fid, vals in samples[group].items():
            if vals.size == 0:
                continue
            sorted_vals = np.sort(vals)
            idx = np.rint(quantiles * (sorted_vals.size - 1)).astype(np.int64)
            boundaries = sorted_vals[idx].astype(np.int64)
            stats[group][str(fid)] = boundaries.tolist()

    logging.info(
        f"Built qbin stats: user_int={len(stats['user_int'])}, "
        f"item_int={len(stats['item_int'])}, num_bins={num_bins}, "
