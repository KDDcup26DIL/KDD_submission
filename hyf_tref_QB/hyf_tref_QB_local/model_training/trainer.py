...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        qbin_stats_path: Optional[str] = None,
        ...
        self.qbin_stats_path: Optional[str] = qbin_stats_path
        ...
        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     ...
                     f"bpr_weight={getattr(model, 'bpr_weight', 0.0)}")
    ...
    def _write_sidecar_files(self, ckpt_dir: str) -> None:
        ...
        Currently persists sidecar files, all overwritten on every call:
        ...
        qbin_stats_copied = False
        if self.qbin_stats_path and os.path.exists(self.qbin_stats_path):
            shutil.copy2(self.qbin_stats_path, ckpt_dir)
            qbin_stats_copied = True

        if self.train_config:
            import json
            cfg_to_dump = self.train_config
            if ns_groups_copied or qbin_stats_copied:
            ...
            if ns_groups_copied:
                cfg_to_dump['ns_groups_json'] = os.path.basename(
                    self.ns_groups_path)
            if qbin_stats_copied:
                cfg_to_dump['qbin_stats_json'] = os.path.basename(
                    self.qbin_stats_path)
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        graph_user_ids = device_batch.get('graph_user_ids')
        graph_item_ids = device_batch.get('graph_item_ids')
        outputs = self.model(
            model_input,
        ...
        bpr_loss = outputs.get('bpr_loss')
        if bpr_loss is not None:
            loss = loss + getattr(self.model, 'bpr_weight', 0.0) * bpr_loss
        loss.backward()
