...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Two-tower fusion does not use RankMixer's full-token reshape, so it
        # does not require d_model to be divisible by num_queries*S+num_ns.
        ...
        # ================== Two HyFormer Towers ==================
        # non-seq tower: real NS tokens + empty sequence placeholders.
        # seq tower: real sequence tokens + learned NS seed tokens.
        def _make_query_generator() -> nn.Module:
            return MultiSeqQueryGenerator(
                d_model=d_model,
                num_ns=self.num_ns,
        ...
        def _make_output_proj() -> nn.Module:
            return nn.Sequential(
                nn.Linear(num_queries * self.num_sequences * d_model, d_model),
                nn.LayerNorm(d_model),
            )

        self.nonseq_query_generator = _make_query_generator()
        self.seq_query_generator = _make_query_generator()
        self.nonseq_blocks = _make_blocks()
        self.seq_blocks = _make_blocks()
        self.nonseq_output_proj = _make_output_proj()
        self.seq_output_proj = _make_output_proj()
        self.seq_tower_ns = nn.Parameter(torch.zeros(1, self.num_ns, d_model))
    ...
    def _run_hyformer_tower(
        self,
        query_generator: nn.Module,
        ...
        """Run one full HyFormer tower and return its d_model representation."""
        q_tokens_list = query_generator(ns_tokens, seq_tokens_list, seq_masks_list)
        ...
        return output_proj(all_q.view(B, -1))

    def forward(self, inputs: ModelInput) -> torch.Tensor:
        """Runs the two-tower HyFormer forward pass."""
        ...
        B = ns_tokens.shape[0]
        dummy_seq_tokens = [
            ns_tokens.new_zeros(B, 1, self.d_model)
            for _ in self.seq_domains
        ]
        dummy_seq_masks = [
            torch.ones(B, 1, dtype=torch.bool, device=ns_tokens.device)
            for _ in self.seq_domains
        ]
        nonseq_repr = self._run_hyformer_tower(
            self.nonseq_query_generator,
            self.nonseq_blocks,
            self.nonseq_output_proj,
            ns_tokens,
            dummy_seq_tokens,
            dummy_seq_masks,
        ...
        seq_ns_tokens = self.seq_tower_ns.expand(B, -1, -1)
        seq_repr = self._run_hyformer_tower(
            self.seq_query_generator,
            self.seq_blocks,
            self.seq_output_proj,
            seq_ns_tokens,
            seq_tokens_list,
            seq_masks_list,
            apply_dropout=self.training,
        )
        output = self.fusion(torch.cat([nonseq_repr, seq_repr], dim=-1))
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        """Runs two-tower inference, returning both logits and fused embeddings."""
