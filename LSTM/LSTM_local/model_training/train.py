"""PCVRLSTM training entry point (self-contained baseline).
...
from model import PCVRLSTM
from trainer import PCVRLSTMRankingTrainer
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PCVRLSTM Training")
    ...
    parser.add_argument('--num_lstm_layers', '--num_hyformer_blocks', dest='num_lstm_layers', type=int, default=2,
                        help='Number of LSTM layers per sequence domain')
...
def main() -> None:
    ...
    model = PCVRLSTM(**model_args).to(args.device)

    # Log model sizing info.
    num_sequences = len(pcvr_dataset.seq_domains)
    num_ns = model.num_ns
    T = args.num_queries * num_sequences + num_ns
    logging.info(f"PCVRLSTM model created: num_ns={num_ns}, T={T}, d_model={args.d_model}, rank_mixer_mode={args.rank_mixer_mode}")
    ...
    ckpt_params = {
        "layer": args.num_lstm_layers,
        "head": args.num_heads,
        "hidden": args.d_model,
    }

    trainer = PCVRLSTMRankingTrainer(
        model=model,
