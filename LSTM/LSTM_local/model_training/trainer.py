"""PCVRLSTM pointwise trainer (binary-classification, AUC-monitored).
...
class PCVRLSTMRankingTrainer:
    """PCVRLSTM trainer for pointwise binary classification.
    ...
    def __init__(
        ...
        logging.info(f"PCVRLSTMRankingTrainer loss_type={loss_type}, "
    ...
    def evaluate(self, epoch: Optional[int] = None) -> Tuple[float, float]:
        ...
        """
        print("Start Evaluation (PCVRLSTM) - validation")
        self.model.eval()
        if not epoch:
            epoch = -1
