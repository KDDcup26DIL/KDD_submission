"""PCVRLSTM inference script (uploaded by the contestant into the
...
from model import PCVRLSTM, ModelInput
...
# ``train_config.json`` is ignored when constructing ``PCVRLSTM``.
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        ...
        elif key == 'num_lstm_layers' and 'num_lstm_layers' in train_config:
            cfg[key] = train_config['num_lstm_layers']
        else:
            cfg[key] = _FALLBACK_MODEL_CFG[key]
            logging.warning(
...
def build_model(
    ...
    """Construct a ``PCVRLSTM`` from the dataset schema, an NS-groups JSON,
    ...
    logging.info(f"Building PCVRLSTM with cfg: {model_cfg}")
    model = PCVRLSTM(
