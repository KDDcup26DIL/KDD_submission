...
class PCVRHyFormer(nn.Module):
    ...
    def _project_q_outputs(self, curr_qs: list) -> torch.Tensor:
        """Projects concatenated per-domain query tokens to the classifier space."""
        B = curr_qs[0].shape[0]
    ...
    def _run_multi_seq_blocks(
        ...
        """Runs the block stack and optionally returns intermediate outputs."""
        if apply_dropout:
        ...
        for block_idx, block in enumerate(self.blocks):
            ...
            if return_aux_outputs and block_idx < len(self.blocks) - 1:
                aux_outputs.append(self._project_q_outputs(curr_qs))

        output = self._project_q_outputs(curr_qs)
    ...
    def forward(
        ...
        block_result = self._run_multi_seq_blocks(
            q_tokens_list, ns_tokens, seq_tokens_list, seq_masks_list,
            apply_dropout=self.training,
            return_aux_outputs=return_aux,
        )

        # 5. Classifier
        if return_aux:
            output, aux_outputs = block_result
            logits = self.clsfier(output)  # (B, action_num)
            aux_logits = [self.clsfier(aux_output) for aux_output in aux_outputs]
