...
class PCVRHyFormerRankingTrainer:
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        model_out = self.model(model_input, return_aux=self.aux_loss_weight > 0)
        if isinstance(model_out, tuple):
            logits, aux_logits_list = model_out
        else:
            logits, aux_logits_list = model_out, []
        ...
        if self.aux_loss_weight > 0 and aux_logits_list:
            aux_losses = []
            for aux_logits in aux_logits_list:
                aux_logits = aux_logits.squeeze(-1)
                if self.loss_type == 'focal':
                    aux_loss = sigmoid_focal_loss(aux_logits, label, alpha=self.focal_alpha, gamma=self.focal_gamma)
                else:
                    aux_loss = F.binary_cross_entropy_with_logits(aux_logits, label)
