"""PCVRMLP pointwise trainer (binary-classification, AUC-monitored).
...
class PCVRMLPRankingTrainer:
    """PCVRMLP trainer for pointwise binary classification.
    ...
    def __init__(
        ...
        logging.info(f"PCVRMLPRankingTrainer loss_type={loss_type}, "
    ...
    def evaluate(self, epoch: Optional[int] = None) -> Tuple[float, float]:
        ...
        """
        print("Start Evaluation (PCVRMLP) - validation")
        self.model.eval()
        if not epoch:
            epoch = -1
