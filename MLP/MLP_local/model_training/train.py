"""PCVRMLP training entry point (self-contained baseline).
...
from model import PCVRMLP
from trainer import PCVRMLPRankingTrainer
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PCVRMLP Training")
    ...
    parser.add_argument('--num_mlp_layers', '--num_hyformer_blocks', dest='num_mlp_layers', type=int, default=2,
...
def main() -> None:
    ...
    model = PCVRMLP(**model_args).to(args.device)

    # Log model sizing info.
    num_sequences = len(pcvr_dataset.seq_domains)
    num_ns = model.num_ns
    T = args.num_queries * num_sequences + num_ns
    logging.info(f"PCVRMLP model created: num_ns={num_ns}, T={T}, d_model={args.d_model}, rank_mixer_mode={args.rank_mixer_mode}")
    ...
    ckpt_params = {
        "layer": args.num_mlp_layers,
        "head": args.num_heads,
        "hidden": args.d_model,
    }

    trainer = PCVRMLPRankingTrainer(
        model=model,
