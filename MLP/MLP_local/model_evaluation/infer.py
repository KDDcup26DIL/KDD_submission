"""PCVRMLP inference script (uploaded by the contestant into the
...
from model import PCVRMLP, ModelInput
...
# ``train_config.json`` is ignored when constructing ``PCVRMLP``.
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        ...
        elif key == 'num_mlp_layers' and 'num_mlp_layers' in train_config:
            cfg[key] = train_config['num_mlp_layers']
        else:
            cfg[key] = _FALLBACK_MODEL_CFG[key]
            logging.warning(
...
def build_model(
    ...
    """Construct a ``PCVRMLP`` from the dataset schema, an NS-groups JSON,
    ...
    logging.info(f"Building PCVRMLP with cfg: {model_cfg}")
    model = PCVRMLP(
