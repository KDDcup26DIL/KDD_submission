...
class PCVRMLP(nn.Module):
    """Raw MLP baseline over pooled sparse/dense/sequence features."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        layers: List[nn.Module] = [nn.Linear(input_dim, hidden_dim), nn.ReLU(), nn.LayerNorm(hidden_dim), nn.Dropout(dropout_rate)]
        for _ in range(max(0, num_mlp_layers - 1)):
            layers.extend([nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.LayerNorm(hidden_dim), nn.Dropout(dropout_rate)])
        layers.append(nn.Linear(hidden_dim, action_num))
        self.net = nn.Sequential(*layers)
        self.reset_parameters()
        logging.info(f"PCVRMLP created: input_dim={input_dim}, layers={num_mlp_layers}, hidden_dim={hidden_dim}")
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        return self.net(self._features(inputs))
