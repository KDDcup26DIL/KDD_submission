...
class UniMixerNSTokenizer(nn.Module):
    ...
    def __init__(
        ...
        # RankUp-style randomized splitting prior.  The permutation is
        # deterministic and local to this tokenizer, so train/eval/submission
        # build the same field-to-token assignment without any data leakage.
        base_order = [fid_idx for group in groups for fid_idx in group]
        if base_order:
            perm_seed = 20260519 + len(base_order) * 997 + num_ns_tokens * 131
            perm = torch.randperm(len(base_order), generator=torch.Generator().manual_seed(perm_seed)).tolist()
            self.field_order = [base_order[i] for i in perm]
        else:
            self.field_order = base_order
        ...
        if self.num_fields > 0:
            with torch.no_grad():
                ...
                chunk_dim = math.ceil(self.num_fields / num_ns_tokens)
                for token_idx in range(num_ns_tokens):
                    start = token_idx * chunk_dim
                    end = min(start + chunk_dim, self.num_fields)
                    if start < end:
                        self.mix_logits[token_idx, start:end].fill_(2.0)
        self.token_bias = nn.Parameter(torch.zeros(num_ns_tokens, d_model))
        self.out_norm = nn.LayerNorm(d_model)

        logging.info(
            f"UniMixerNSTokenizer RankUpSplit: {self.num_fields} fids, "
            f"num_ns_tokens={num_ns_tokens}, emb_dim={emb_dim}, d_model={d_model}, "
            f"chunk_init={math.ceil(max(1, self.num_fields) / num_ns_tokens)}"
        )
