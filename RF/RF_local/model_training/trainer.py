"""PCVRRF pointwise trainer (binary-classification, AUC-monitored).
...
class PCVRRFRankingTrainer:
    """PCVRRF trainer for pointwise binary classification.
    ...
    def __init__(
        ...
        logging.info(f"PCVRRFRankingTrainer loss_type={loss_type}, "
    ...
    def evaluate(self, epoch: Optional[int] = None) -> Tuple[float, float]:
        ...
        """
        print("Start Evaluation (PCVRRF) - validation")
        self.model.eval()
        if not epoch:
            epoch = -1
