"""PCVRRF training entry point (self-contained baseline).
...
from model import PCVRRF
from trainer import PCVRRFRankingTrainer
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PCVRRF Training")
    ...
    parser.add_argument('--num_rf_layers', '--num_hyformer_blocks', dest='num_rf_layers', type=int, default=2,
                        help='Random-feature width multiplier')
...
def main() -> None:
    ...
    model_args = {
        ...
        "num_rf_layers": args.num_rf_layers,
    ...
    model = PCVRRF(**model_args).to(args.device)

    # Log model sizing info.
    num_sequences = len(pcvr_dataset.seq_domains)
    num_ns = model.num_ns
    T = args.num_queries * num_sequences + num_ns
    logging.info(f"PCVRRF model created: num_ns={num_ns}, T={T}, d_model={args.d_model}, rank_mixer_mode={args.rank_mixer_mode}")
    ...
    ckpt_params = {
        "layer": args.num_rf_layers,
        "head": args.num_heads,
        "hidden": args.d_model,
    }

    trainer = PCVRRFRankingTrainer(
        model=model,
