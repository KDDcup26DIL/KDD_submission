...
class PCVRRF(nn.Module):
    """Raw random-feature baseline over pooled sparse/dense/sequence features."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.seq_blocks = nn.ModuleDict({
            domain: SequenceMeanBlock(vocab_sizes, emb_dim, emb_skip_threshold)
            for domain, vocab_sizes in seq_vocab_sizes.items()
        })

        input_dim = (len(user_int_feature_specs) + len(item_int_feature_specs)) * emb_dim
        input_dim += user_dense_dim + item_dense_dim
        input_dim += sum(len(seq_vocab_sizes[d]) * emb_dim for d in self.seq_domains)

        self.rf_dim = max(d_model, d_model * hidden_mult * max(1, num_rf_layers))
        self.random_projection = nn.Linear(input_dim, self.rf_dim)
        self.norm = nn.LayerNorm(self.rf_dim)
        self.dropout = nn.Dropout(dropout_rate)
        self.head = nn.Linear(self.rf_dim, action_num)
        self.reset_parameters()
        for param in self.random_projection.parameters():
            param.requires_grad = False
        logging.info(f"PCVRRF created: input_dim={input_dim}, rf_dim={self.rf_dim}")
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        feats = self.dropout(self.norm(F.relu(self.random_projection(self._features(inputs)))))
        return self.head(feats)

    def predict(self, inputs: ModelInput):
        raw_feats = self._features(inputs)
        feats = self.dropout(self.norm(F.relu(self.random_projection(raw_feats))))
        return self.head(feats), raw_feats
    ...
    def get_dense_params(self) -> List[nn.Parameter]:
        ...
        return [param for param in self.parameters() if id(param) not in sparse_ids and param.requires_grad]
