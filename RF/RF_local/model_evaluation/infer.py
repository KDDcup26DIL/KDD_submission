"""PCVRRF inference script (uploaded by the contestant into the
...
from model import PCVRRF, ModelInput
...
# ``train_config.json`` is ignored when constructing ``PCVRRF``.
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        ...
        elif key == 'num_rf_layers' and 'num_rf_layers' in train_config:
            cfg[key] = train_config['num_rf_layers']
        else:
            cfg[key] = _FALLBACK_MODEL_CFG[key]
            logging.warning(
...
def build_model(
    ...
    """Construct a ``PCVRRF`` from the dataset schema, an NS-groups JSON,
    ...
    logging.info(f"Building PCVRRF with cfg: {model_cfg}")
    model = PCVRRF(
