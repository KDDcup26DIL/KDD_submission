...
class MultiSeqQueryGenerator(nn.Module):
    ...
    def __init__(
        ...
        num_sequences: int,
        hidden_mult: int = 4,
        query_context_mode: str = 'ns_seq',
    ) -> None:
        super().__init__()
        if query_context_mode not in {'ns_seq', 'ns_only', 'seq_only', 'ns_zero_seq'}:
            raise ValueError(f"Unknown query_context_mode: {query_context_mode}")
        self.num_queries = num_queries
        self.num_sequences = num_sequences
        self.d_model = d_model
        self.query_context_mode = query_context_mode

        if query_context_mode == 'seq_only':
            global_info_dim = d_model
        elif query_context_mode == 'ns_only':
            global_info_dim = num_ns * d_model
        else:
    ...
    def forward(
        ...
        for i in range(self.num_sequences):
            ...
            if self.query_context_mode == 'seq_only':
                global_info = seq_pooled
            elif self.query_context_mode == 'ns_only':
                global_info = ns_flat
            elif self.query_context_mode == 'ns_zero_seq':
                global_info = torch.cat([ns_flat, torch.zeros_like(seq_pooled)], dim=-1)
            else:
