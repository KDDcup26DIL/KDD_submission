...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        # TokenFormer-style NLIR gate.  It starts as exact passthrough because
        # nlir_scale is initialized to zero, then learns how strongly each
        # query channel should keep/amplify the sequence attention output.
        self.nlir_gates = nn.ModuleList([
            nn.Sequential(
                nn.LayerNorm(d_model),
        ...
        self.nlir_scale = nn.Parameter(torch.zeros(1))
    ...
    def forward(
        ...
        for i in range(S):
            ...
            decoded_q_i = self.cross_attns[i](
                q_i, next_seqs[i], next_masks[i],
                rope_cos=rc, rope_sin=rs,
            )
            gate_i = torch.sigmoid(self.nlir_gates[i](q_i))
            decoded_q_i = decoded_q_i * (1.0 + self.nlir_scale.to(dtype=decoded_q_i.dtype) * (gate_i - 0.5))
