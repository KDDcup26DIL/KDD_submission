...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # ================== Exact Two HyFormer Towers ==================
        # Both towers receive the same real NS tokens and the same real sequence
        # tokens. They have identical architecture but independent parameters.
        ...
        self.tower1_query_generator = _make_query_generator()
        self.tower2_query_generator = _make_query_generator()
        self.tower1_blocks = _make_blocks()
        self.tower2_blocks = _make_blocks()
        self.tower1_output_proj = _make_output_proj()
        self.tower2_output_proj = _make_output_proj()
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        """Runs two identical-input HyFormer towers."""
        ...
        tower1_repr = self._run_hyformer_tower(
            self.tower1_query_generator,
            self.tower1_blocks,
            self.tower1_output_proj,
            ns_tokens,
            seq_tokens_list,
            seq_masks_list,
        ...
        tower2_repr = self._run_hyformer_tower(
            self.tower2_query_generator,
            self.tower2_blocks,
            self.tower2_output_proj,
            ns_tokens,
            seq_tokens_list,
            seq_masks_list,
            apply_dropout=self.training,
        )
        output = self.fusion(torch.cat([tower1_repr, tower2_repr], dim=-1))
