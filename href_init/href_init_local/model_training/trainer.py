...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        self.micro_batch_size: int = int(micro_batch_size or 0)
    ...
    def _slice_batch(
        ...
        """Slice tensors/lists along the batch dimension for micro-batching."""
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        micro = self.micro_batch_size if self.micro_batch_size > 0 else batch_size
        micro = max(1, min(micro, batch_size))
        total_loss = 0.0

        for start in range(0, batch_size, micro):
            end = min(start + micro, batch_size)
            micro_batch = self._slice_batch(device_batch, start, end, batch_size)
            micro_label = label[start:end]
            model_input = self._make_model_input(micro_batch)
            ...
            if self.loss_type == 'focal':
                ...
                loss = F.binary_cross_entropy_with_logits(logits, micro_label)
    ...
    def _evaluate_step(
        ...
        for start in range(0, batch_size, micro):
            ...
            logits_parts.append(logits.squeeze(-1))
