"""PCVRHyFormerDCN training entry point (self-contained baseline).
...
from model import PCVRHyFormerDCN
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PCVRHyFormerDCN Training")
    ...
    parser.add_argument('--fusion_hidden', type=int, default=128,
                        help='Legacy hidden dimension kept for checkpoint compatibility')
    parser.add_argument('--fusion_rank', type=int, default=16,
                        help='Low-rank dimension for bilinear fusion')
    parser.add_argument('--fusion_heads', type=int, default=4,
                        help='Number of bilinear fusion heads')
    parser.add_argument('--dcn_lite_dim', type=int, default=64,
                        help='Compressed dimension for the small DCNv2-lite tower')
    parser.add_argument('--dcn_lite_layers', type=int, default=1,
                        help='Number of CrossNet layers in the DCNv2-lite tower')
    parser.add_argument('--dcn_lite_hidden', type=int, default=64,
                        help='Hidden dimension of the DCNv2-lite parallel MLP')
    parser.add_argument('--action_num', type=int, default=1,
                        help='Classifier output dimension '
                             '(1 = single binary-classification logit; >1 = multi-label)')

    # Compatibility-only flags accepted by HyFormer/tref run configs. They are
    # intentionally ignored in this WuKong two-tower implementation so shared
    # launch commands do not fail argument parsing.
    parser.add_argument('--ns_tokenizer_type', type=str, default=None,
                        help=argparse.SUPPRESS)
    parser.add_argument('--user_ns_tokens', type=int, default=None,
                        help=argparse.SUPPRESS)
    parser.add_argument('--item_ns_tokens', type=int, default=None,
                        help=argparse.SUPPRESS)
...
def main() -> None:
    ...
    model_args = {
        ...
        "dcn_lite_dim": args.dcn_lite_dim,
        "dcn_lite_layers": args.dcn_lite_layers,
        "dcn_lite_hidden": args.dcn_lite_hidden,
        "action_num": args.action_num,
        "emb_skip_threshold": args.emb_skip_threshold,
    }

    model = PCVRHyFormerDCN(**model_args).to(args.device)

    # Log model sizing info.
    logging.info(
        f"PCVRHyFormerDCN model created: d_model={args.d_model}, emb_dim={args.emb_dim}, "
