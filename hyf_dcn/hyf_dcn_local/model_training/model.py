...
class SimpleFusion(nn.Module):
    """Simple two-tower fusion for HyFormer and DCNv2-lite.

    This mirrors the original HyFormer+WuKong simple fusion style but uses the
    DCNv2-lite auxiliary tower instead of WuKong:

        fused = [hy ; dcn ; hy * dcn ; |hy - dcn|]
        logit = MLP(fused)
    """
    ...
    def _fused(self, hy: torch.Tensor, dcn: torch.Tensor) -> torch.Tensor:
        return torch.cat([hy, dcn, hy * dcn, torch.abs(hy - dcn)], dim=-1)

    def forward(self, hy: torch.Tensor, dcn: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        fused = self._fused(hy, dcn)
        logits = self.net(fused)
        rep = self.rep_proj(fused)
        return logits, rep

class PCVRHyFormerDCN(nn.Module):
    """Two-tower model: HyFormer-style transformer tower + DCNv2-lite tower with simple fusion."""
    ...
    def __init__(
        ...
        logging.info(
            "PCVRHyFormerDCN created: fields=%d, d_model=%d, emb_dim=%d, "
            "hyformer_layers=%d, dcn_dim=%d, dcn_layers=%d, "
            "simple_fusion_hidden=%d",
            num_fields, d_model, emb_dim, num_hyformer_layers,
            dcn_lite_dim, dcn_lite_layers, fusion_hidden,
        )
...
PCVRHyFormerDCNBifusion = PCVRHyFormerDCN
PCVRHyFormerWuKong = PCVRHyFormerDCN
PCVRDCNv2 = PCVRHyFormerDCN
