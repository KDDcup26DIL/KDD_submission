...
class UniMixerBlock(nn.Module):
    """Learnable token mixer used as a RankMixerBlock replacement.

    RankMixer rewires channels with a fixed reshape/transpose pattern. This block
    keeps the same input/output contract, but learns how much every decoded query
    or NS token should read from every other token. A residual identity bias keeps
    the initial behaviour close to passthrough.
    """
    ...
    def __init__(
        ...
        self.T = n_total
        self.D = d_model
        self.mix_logits = nn.Parameter(torch.zeros(n_total, n_total))
        self.value_proj = nn.Linear(d_model, d_model, bias=False)
        self.mix_gate = nn.Parameter(torch.tensor(init_scale))
    ...
    def forward(self, Q: torch.Tensor) -> torch.Tensor:
        if Q.shape[1] != self.T:
            raise ValueError(f"UniMixerBlock expected T={self.T}, got {Q.shape[1]}")

        eye = torch.eye(self.T, device=Q.device, dtype=Q.dtype)
        mix = F.softmax(self.mix_logits.to(dtype=Q.dtype) + eye * 2.0, dim=-1)
        mixed = torch.einsum('ij,bjd->bid', mix, self.value_proj(Q))
        x = Q + self.mix_gate.to(dtype=Q.dtype) * mixed
...
class RankMixerBlock(nn.Module):
    ...
    def __init__(
        ...
        hidden_mult: int = 4,
        dropout: float = 0.0,
        mode: str = 'full'  # 'full' | 'ffn_only' | 'none' | 'unimixer'
    ) -> None:
        super().__init__()
        self.T = n_total
        self.D = d_model
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        if rank_mixer_mode == 'unimixer':
            self.mixer = UniMixerBlock(
                d_model=d_model,
                n_total=n_total,
                hidden_mult=hidden_mult,
                dropout=dropout,
...
class UniMixerNSTokenizer(nn.Module):
    """NS tokenizer with learnable feature-token mixing.

    Each discrete fid first becomes a field token. The tokenizer then learns a
    dense mixing matrix from field tokens to a fixed number of NS tokens, so the
    token boundaries are learned instead of being fixed concat/split chunks.
    """
    ...
    def __init__(
        ...
        self.field_order = [fid_idx for group in groups for fid_idx in group]
        self.num_fields = len(self.field_order)
        ...
        self.mix_logits = nn.Parameter(torch.zeros(num_ns_tokens, max(1, self.num_fields)))
        self.token_bias = nn.Parameter(torch.zeros(num_ns_tokens, d_model))
        self.out_norm = nn.LayerNorm(d_model)

        logging.info(
            f"UniMixerNSTokenizer: {self.num_fields} fids, "
            f"num_ns_tokens={num_ns_tokens}, emb_dim={emb_dim}, d_model={d_model}"
        )

    def forward(self, int_feats: torch.Tensor) -> torch.Tensor:
        field_embs = []
        for fid_idx in self.field_order:
            vs, offset, length = self.feature_specs[fid_idx]
            emb_real_idx = self._emb_index[fid_idx]
            if emb_real_idx == -1:
                fid_emb = int_feats.new_zeros(int_feats.shape[0], self.emb_dim)
                ...
                if length == 1:
                    ...
                    count = mask.sum(dim=1).clamp(min=1)
                    fid_emb = (emb_all * mask).sum(dim=1) / count
            field_embs.append(fid_emb)

        if not field_embs:
            return int_feats.new_zeros(int_feats.shape[0], self.num_ns_tokens, self.out_norm.normalized_shape[0]).float()

        fields = torch.stack(field_embs, dim=1)  # (B, F, emb_dim)
        fields = F.silu(self.field_proj(fields))  # (B, F, D)
        weights = F.softmax(self.mix_logits[:, :fields.shape[1]].to(dtype=fields.dtype), dim=-1)
        tokens = torch.einsum('tf,bfd->btd', weights, fields)
        tokens = tokens + self.token_bias.to(dtype=tokens.dtype).unsqueeze(0)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        elif ns_tokenizer_type in ('rankmixer', 'unimixer'):
            # RankMixer paper style: all embeddings cat → split → project.
            # UniMixer style: embed fids as field tokens → learned mix to NS tokens.
            # 0 means auto: fall back to group count.
            if user_ns_tokens <= 0:
                user_ns_tokens = len(user_ns_groups)
            if item_ns_tokens <= 0:
                item_ns_tokens = len(item_ns_groups)
            tokenizer_cls = UniMixerNSTokenizer if ns_tokenizer_type == 'unimixer' else RankMixerNSTokenizer
        ...
        # Total NS token count.  One extra match token is appended to
        # the block input, but not used by the query generator summaries.
        ...
        self.match_token_proj = nn.Sequential(
    ...
    def _mean_pool_sequences(
        self,
        seq_tokens_list: list,
        seq_masks_list: list,
    ) -> torch.Tensor:
        """Mean-pools all domain sequence tokens into one user-interest vector."""
        pooled = []
        for tokens, mask in zip(seq_tokens_list, seq_masks_list):
            valid = (~mask).unsqueeze(-1).float()
            seq_sum = (tokens * valid).sum(dim=1)
            seq_count = valid.sum(dim=1).clamp(min=1)
            pooled.append(seq_sum / seq_count)
        return torch.stack(pooled, dim=1).mean(dim=1)

    def _make_match_token(
        self,
        seq_tokens_list: list,
        seq_masks_list: list,
        item_tokens: torch.Tensor,
    ) -> torch.Tensor:
        """Builds a feature-only user-interest/item match token."""
        seq_summary = self._mean_pool_sequences(seq_tokens_list, seq_masks_list)
        item_summary = item_tokens.mean(dim=1)
        match_features = torch.cat(
            [
                seq_summary,
                item_summary,
                seq_summary * item_summary,
                torch.abs(seq_summary - item_summary),
            ],
            dim=-1,
        )
        return self.match_token_proj(match_features).unsqueeze(1)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        match_token = self._make_match_token(
            seq_tokens_list=seq_tokens_list,
            seq_masks_list=seq_masks_list,
            item_tokens=refined_item,
        )
        ns_tokens = torch.cat([ns_tokens, match_token], dim=1)
