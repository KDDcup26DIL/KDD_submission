...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        self.swa_param_names = [
            name for name, param in self.model.named_parameters()
            if param.requires_grad and param.is_floating_point()
            and param.data_ptr() not in self._sparse_param_ptrs
        ]

        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     f"focal_alpha={focal_alpha}, focal_gamma={focal_gamma}, "
                     f"reinit_sparse_after_epoch={reinit_sparse_after_epoch}, "
                     f"swa_start_epoch={swa_start_epoch}, "
                     f"swa_dense_params={len(self.swa_param_names)}")
    ...
    def _update_dense_swa(self, epoch: int) -> None:
        """Accumulate dense-parameter SWA state on CPU."""
        if self.swa_start_epoch <= 0 or epoch < self.swa_start_epoch:
            return
        named_params = dict(self.model.named_parameters())
        self.swa_n += 1
        for name in self.swa_param_names:
            param = named_params[name].detach().float().cpu()
            if name not in self.swa_state:
                self.swa_state[name] = param.clone()
            else:
                self.swa_state[name].add_((param - self.swa_state[name]) / self.swa_n)
        logging.info(f"Updated dense SWA state: samples={self.swa_n}, epoch={epoch}")

    def _save_dense_swa_checkpoint(self, total_step: int) -> None:
        """Save a single-model checkpoint with best sparse params and SWA dense params."""
        if self.swa_start_epoch <= 0 or self.swa_n == 0:
            return
        best_model_path = self.early_stopping.checkpoint_path
        if not best_model_path or not os.path.exists(best_model_path):
            logging.warning("Dense SWA skipped: best checkpoint path is unavailable")
            return

        best_state = torch.load(best_model_path, map_location='cpu')
        replaced = 0
        for name, avg in self.swa_state.items():
            if name in best_state and torch.is_tensor(best_state[name]) and best_state[name].shape == avg.shape:
                best_state[name] = avg.to(dtype=best_state[name].dtype)
                replaced += 1

        swa_dir = os.path.join(self.save_dir, f"global_step{total_step}.swa_dense.best_model")
        self._remove_old_best_dirs()
        os.makedirs(swa_dir, exist_ok=True)
        torch.save(best_state, os.path.join(swa_dir, "model.pt"))
        self._write_sidecar_files(swa_dir)
        self.early_stopping.checkpoint_path = os.path.join(swa_dir, "model.pt")
        logging.info(
            f"Saved dense-only SWA checkpoint to {swa_dir}/model.pt "
            f"(samples={self.swa_n}, dense_tensors={replaced}, sparse tensors kept from best checkpoint)"
        )
            ...
            for step, batch in train_pbar:
                ...
                if self.eval_every_n_steps > 0 and total_step % self.eval_every_n_steps == 0:
                    ...
                    if self.early_stopping.early_stop:
                        ...
                        self._save_dense_swa_checkpoint(total_step)
                        return
