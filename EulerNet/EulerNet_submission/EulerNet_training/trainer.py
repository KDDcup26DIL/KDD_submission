"""EulerNet pointwise trainer (binary-classification, AUC-monitored).
...
class EulerNetRankingTrainer:
    """EulerNet trainer for pointwise binary classification.
    ...
    def __init__(
        ...
        logging.info(f"EulerNetRankingTrainer loss_type={loss_type}, "
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        if hasattr(self.model, 'get_dense_params'):
            clip_params = self.model.get_dense_params()
        else:
            clip_params = self.model.parameters()
        torch.nn.utils.clip_grad_norm_(clip_params, max_norm=1.0, foreach=False)
    ...
    def evaluate(self, epoch: Optional[int] = None) -> Tuple[float, float]:
        ...
        """
        print("Start Evaluation (EulerNet) - validation")
        self.model.eval()
        if not epoch:
            epoch = -1
