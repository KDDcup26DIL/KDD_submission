...
class MultiSeqQueryGenerator(nn.Module):
    ...
    For each sequence i:
        GlobalInfo_i = Concat(Flatten(NS), F_u, F_i, MeanPool(Seq_i))
    ...
    def __init__(
        ...
        global_info_dim = (num_ns + 3) * d_model
    ...
    def forward(
        ...
        Args:
            ns_tokens: (B, M, D), original shared NS tokens.
        ...
        for i in range(self.num_sequences):
            ...
            global_info = torch.cat(
                [ns_flat, user_summary, item_summary, seq_pooled], dim=-1
            )  # (B, (M+3)*D)
...
class PCVRHyFormer(nn.Module):
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        q_tokens_list = self.query_generator(
            ns_tokens, refined_user, refined_item, seq_tokens_list, seq_masks_list)
