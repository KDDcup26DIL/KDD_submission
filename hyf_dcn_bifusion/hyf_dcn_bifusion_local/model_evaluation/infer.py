"""PCVRHyFormerDCNBifusion inference script (uploaded by the contestant into the
...
from model import PCVRHyFormerDCNBifusion, ModelInput
...
def build_model(
    ...
    device: str = 'cpu',
) -> PCVRHyFormerDCNBifusion:
    """Construct a ``PCVRHyFormerDCNBifusion`` from the dataset schema and model config.
    ...
    logging.info(f"Building PCVRHyFormerDCNBifusion with cfg: {model_cfg}")
    model = PCVRHyFormerDCNBifusion(
