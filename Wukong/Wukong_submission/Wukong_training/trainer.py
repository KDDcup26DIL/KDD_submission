"""PCVRWuKong pointwise trainer (binary-classification, AUC-monitored).
...
class PCVRWuKongRankingTrainer:
    """PCVRWuKong trainer for pointwise binary classification.
    ...
    def __init__(
        ...
        logging.info(f"PCVRWuKongRankingTrainer loss_type={loss_type}, "
    ...
    def train(self) -> None:
        ...
        """
        print("Start training (PCVRWuKong)")
        self.model.train()
        total_step = 0
    ...
    def evaluate(self, epoch: Optional[int] = None) -> Tuple[float, float]:
        ...
        """
        print("Start Evaluation (PCVRWuKong) - validation")
        self.model.eval()
        if not epoch:
            epoch = -1
