"""PCVRWuKong inference script (uploaded by the contestant into the
...
from model import PCVRWuKong, ModelInput
...
# ``train_config.json`` is ignored when constructing ``PCVRWuKong``.
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        ...
        elif key == 'num_wukong_layers' and 'num_hyformer_blocks' in train_config:
...
def build_model(
    ...
    """Construct a ``PCVRWuKong`` from the dataset schema, an NS-groups JSON,
    ...
    logging.info(f"Building PCVRWuKong with cfg: {model_cfg}")
    model = PCVRWuKong(
