"""PCVRWuKong training entry point (self-contained baseline).
...
from model import PCVRWuKong
from trainer import PCVRWuKongRankingTrainer
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PCVRWuKong Training")
    ...
    parser.add_argument('--num_wukong_layers', '--num_hyformer_blocks', dest='num_wukong_layers', type=int, default=3,
                        help='Number of stacked WuKong layers')
...
def main() -> None:
    ...
    model = PCVRWuKong(**model_args).to(args.device)

    # Log model sizing info.
    num_sequences = len(pcvr_dataset.seq_domains)
    num_ns = model.num_ns
    T = args.num_queries * num_sequences + num_ns
    logging.info(f"PCVRWuKong model created: num_ns={num_ns}, T={T}, d_model={args.d_model}, rank_mixer_mode={args.rank_mixer_mode}")
    ...
    ckpt_params = {
        "layer": args.num_wukong_layers,
        "head": args.num_heads,
        "hidden": args.d_model,
    }

    trainer = PCVRWuKongRankingTrainer(
        model=model,
